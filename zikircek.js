...
function anaTabloyuOlustur() {
    document.getElementById("ana-tablo-tbody").innerHTML = "";
    var _satir = "";
    var satir = "";
    for (var i = 0; i < 99; i++) {
        ...
        {
            _satir += '<tr>';
            _satir += '<th scope="row">' + veritabani[i]["id"] + '</th>';
            _satir += '<td>' + veritabani[i]["turkce"] + '<br> ( ' + veritabani[i]["arapca"] + ' ) </td>';
            _satir += '<td hidden></td>';
            _satir += '<td class="text-break"><div style="width:230px;height:1px;"></div>' + veritabani[i]["aciklama"] + '</td>';
            _satir += '</tr>'
        }
        if (parseInt(veritabani[i]["zikirSayisiHedefi"]) == 0) {
...
//         var satir = "";
//         satir += '<tr>';
//         satir += '<th scope="row">' + veritabani[i]["id"] + '</th>';
//         satir += '<td class="fs-3">' + veritabani[i]["turkce"] +'</td>';
//         satir += '<td style="font-size: 40px;">'+veritabani[i]["arapca"] + '</td>';
//         satir += '</tr>';
...
function zikirSayisiniGuncelle(id) {
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[4].textContent = veritabani[id - 1]["cekilenZikirSayisi"];
    ...
    if (parseInt(veritabani[id - 1]["zikirSayisiHedefi"]) == 0) {
        if (parseInt(veritabani[id - 1]['cekilenZikirSayisi']) >= parseInt(veritabani[id - 1]['varsayilanZikirSayisiHedefi'])) {
            document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[0].classList = "bg-success bg-opacity-25";
            ...
            document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[0].classList = "bg-warning bg-opacity-25";
        } else {
            document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[0].classList = "";
        }
    } else {
    ...
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[6].innerHTML = '<td>%' + tamamlanmaYuzdesi + '<div class="progress" style="height: 20px;"><div class="bg-success bg-opacity-75 progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: ' + cubukYuzdesi + '%"></div></div>' + '</td>';
}
function zikirSayisiniArttir() {
    var id = parseInt(document.getElementById("zikir-secme-kutusu").selectedIndex) + 1;
    if (localStorage.getItem("tiklamaSesi") == true) {
        sesCal("/sesler/click-21156.mp3");
        if (parseInt(veritabani[id - 1]["zikirSayisiHedefi"]) > 0) {
            if (parseInt(veritabani[id - 1]["cekilenZikirSayisi"]) + 1 == parseInt(veritabani[id - 1]["zikirSayisiHedefi"])) {
                sesCal("/sesler/zikir-bitirme-sesi.mp3");
            }
        } else {
...
function zikirSayisiHedefiBelirle() {
    ...
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[6].innerHTML = '<td>%' + tamamlanmaYuzdesi + '<div class="progress" style="height: 20px;"><div class="bg-success bg-opacity-75 progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: ' + cubukYuzdesi + '%"></div></div>' + '</td>';
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[5].innerText = yeniZikirSayisiHedefi;
}
...
function cekilenZikirSayisiniDegistir() {
    ...
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[6].innerHTML = '<td>%' + tamamlanmaYuzdesi + '<div class="progress" style="height: 20px;"><div class="bg-success bg-opacity-75 progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width: ' + cubukYuzdesi + '%"></div></div>' + '</td>';
    document.getElementById("ana-tablo-tbody").childNodes[id-1].childNodes[4].innerText = cekilenZikirSayisininYeniDegeri;
}
